...
class SaranKritikController extends Controller
{
    protected function data($pertanyaan)
    {
        $data = Kuesioner_pertanyaan::select([
            ...
            ->where([
                'm_kuesioner_pertanyaan.pertanyaan' => $pertanyaan,
                't_kuesioner.id_dosen' => Auth::guard('dosen')->user()->id_dosen
            ])
    ...
    protected function materi_proses_pembelajaran(Request $request)
    {
        ...
        return view('pages.dosen.saran_kritik.index', compact('title', 'list_tahun_akademik', 'list_semester', 'data'));
    }
